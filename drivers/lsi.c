...
init_table(lsi_private_t *lsi)
{
    ...
    lsi->table->msg_out_ptr = __cpu_to_le32((uintptr_t)&lsi->buffer[LSI_TABLE_MSG_OUT_OFFSET]);
    lsi->table->cmd_ptr = __cpu_to_le32((uintptr_t)&lsi->buffer[LSI_TABLE_CMD_OFFSET]);
    lsi->table->data_in_ptr = __cpu_to_le32((uintptr_t)&lsi->buffer[LSI_TABLE_DATA_OFFSET]);
    lsi->table->status_ptr = __cpu_to_le32((uintptr_t)&lsi->buffer[LSI_TABLE_STATUS_OFFSET]);
    lsi->table->msg_in_ptr = __cpu_to_le32((uintptr_t)&lsi->buffer[LSI_TABLE_MSG_IN_OFFSET]);
    
    /* Set the DSA to point to the base of our data table */
    dsa = (uintptr_t)lsi->table;
...
inquiry(lsi_private_t *lsi, sd_private_t *sd)
{
    ...
    lsi->table->id_addr = __cpu_to_le32(&lsi->scripts[0x2]);
    
    /* Write DSP to start DMA engine */    
    uint32_t dsp = (uintptr_t)lsi->scripts;
...
test_unit_ready(lsi_private_t *lsi, sd_private_t *sd)
{
    ...
    lsi->table->id = __cpu_to_le32((sd->id << 16));
    lsi->table->id_addr = __cpu_to_le32(&lsi->scripts[0x12]);

    /* Write DSP to start DMA engine */    
    uint32_t dsp = (uintptr_t)&lsi->scripts[0x10];
    lsi->mmio[LSI_DSP] = dsp & 0xff;
    lsi->mmio[LSI_DSP + 1] = (dsp >> 8) & 0xff;
    lsi->mmio[LSI_DSP + 2] = (dsp >> 16) & 0xff;
    lsi->mmio[LSI_DSP + 3] = (dsp >> 24) & 0xff;
...
ob_sd_read_sector(lsi_private_t *lsi, sd_private_t *sd, int offset)
{
    ...
    lsi->table->id = __cpu_to_le32((sd->id << 16));
    lsi->table->id_addr = __cpu_to_le32(&lsi->scripts[0x22]);
    
    /* Write DSP to start DMA engine */    
    dsp = (uintptr_t)&lsi->scripts[0x20];
    lsi->mmio[LSI_DSP] = dsp & 0xff;
    lsi->mmio[LSI_DSP + 1] = (dsp >> 8) & 0xff;
    lsi->mmio[LSI_DSP + 2] = (dsp >> 16) & 0xff;
    lsi->mmio[LSI_DSP + 3] = (dsp >> 24) & 0xff;
    ...
    /* Write DSP to start DMA engine */    
    dsp = (uintptr_t)&lsi->scripts[0x2a];
    lsi->mmio[LSI_DSP] = dsp & 0xff;
    lsi->mmio[LSI_DSP + 1] = (dsp >> 8) & 0xff;
    lsi->mmio[LSI_DSP + 2] = (dsp >> 16) & 0xff;
    lsi->mmio[LSI_DSP + 3] = (dsp >> 24) & 0xff;
...
static int
ob_lsi_initialize(lsi_private_t **lsi_p)
{
    ...
    *lsi_p = lsi;
    ...
    /* Buffer for commands */
    lsi->buffer = malloc(4096);
    lsi->scripts = malloc(0x40 * sizeof(uint32_t));
    lsi->table = malloc(sizeof(lsi_table_t));
    
    set_int_property(ph, "#address-cells", 1);
    set_int_property(ph, "#size-cells", 0);
    
    /* Add mapped registers and delete temporary properties */
    value = get_int_property(ph, "_mmio", &len);
    lsi->mmio = cell2pointer(value);
    push_str("_mmio");
...
ob_lsi_init(const char *path, uint64_t mmio, uint64_t ram)
{
    ...
    /* Temporary properties to get the mapped registers */
    set_int_property(ph, "_mmio", (uint32_t)mmio);
    set_int_property(ph, "_ram", (uint32_t)ram);
